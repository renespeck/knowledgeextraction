package org.aksw.simba.knowledgeextraction.commons.mongodb;
...
public class MongoManager {
  ...
  public static MongoManager getMongoManager(final XMLConfiguration config) {
    mc = new MongoClient(config.getString(HOST), config.getInt(PORT));
    return new MongoManager().setConfig(config.getString(NAME), config.getString(COLLECTION));
  }

  public static MongoManager getMongoManager(final String host, final int port, final String name,
      final String collection) {
    mc = new MongoClient(host, port);
    return new MongoManager().setConfig(name, collection);
  }
